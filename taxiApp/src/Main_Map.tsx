...
function Main_Map() {
  ...
  const callTaxi = async () => {
    ...
    if (!(startAddr && endAddr)) {
      Alert.alert('알림', '출발지 또는 도착지가 모두 입력되어야 합니다.', [
  ...
  const setMyLocation = () => {
    ...
    Geolocation.getCurrentPosition(
      ...
      error => {
        setLoading(false);
        console.log('Geolocation Error / error = ' + JSON.stringify(error));
      },
      {
  ...
  return (
    <SafeAreaView style={styles.container}>
        ...
        <View style={{ position: 'absolute', padding: wp(2) }}>
          <View style={{ width: wp(75) }}>
            <GooglePlacesAutocomplete
              ...
              textInputProps={{
                onFocus: () => console.log('입력창에 포커스가 맞춰졌습니다.'),
                // 다른 TextInput 관련 props도 이 안에 추가할 수 있습니다.
                // (예: placeholder, onBlur, style 등)
              }}
            />
          </View>
          <View style={{ width: wp(75) }}>
      ...
      {/* showBtn 변수의 값이 true일 때만 오른쪽에 있는 JSX 요소(...)를 화면에 그려달라 */}
      {showBtn && (
        <View
          style={{
            position: 'absolute',
